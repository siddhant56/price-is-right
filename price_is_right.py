...
class App:
    ...
    def run(self):
        with gr.Blocks(title="The Price is Right", fill_width=True) as ui:
            ...
            with gr.Row():
                scan_button = gr.Button("Scan For Deals", variant="primary")
                clear_button = gr.Button("Clear Deals", variant="secondary")
            with gr.Row():
            ...
            def clear_deals():
                self.get_agent_framework().clear_memory()
                return [], html_for([]), []

            ui.load(load_saved_deals, outputs=[opportunities_dataframe])
